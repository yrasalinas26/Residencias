...
def renderizar_recibos():
    ...
    mes_recibo_gral = st.text_input(
        ...
        key="periodo_recibo_general_input",  # <-- Clave única para evitar duplicados en Streamlit
    )
    ...
    try:
        ...
        if gastos_mes_df.empty:
            st.warning(
                f"⚠️ No se encontraron gastos aprobados para el periodo {mes_recibo_gral}. Por eso el desglose sale vacío."
            )
        else:
            st.success(
        ...
        for _, u_row in unidades_df.iterrows():
            ...
            with st.expander(
                ...
                if cargos_apt > 0:
                    msg_ind += (
                        f"• Cargos Extras / No Comunes: ${cargos_apt:,.2f}\n"
                    )
        ...
        if not gastos_mes_df.empty:
            ...
            texto_ws += (
                f"📄 *RESUMEN DE GASTOS Y COBRANZA - {mes_recibo_gral}*\n\n"
            )
            ...
            for _, u_row in unidades_df.iterrows():
                ...
                texto_ws += (
                    f"• Apto {u_cod} ({u_alic:.1f}%): ${cuota_apt_gral:,.2f}\n"
                )
...
# 3. PANEL DE PROPIETARIO
    ...
    with t_p2:
        ...
        st.info(
            "ℹ️ Si pagas en Bolívares (VES), ingresa el monto en Bs. La aplicación buscará automáticamente la tasa del dólar oficial BCV correspondiente a la fecha de tu pago y lo convertirá a dólares ($)."
        )
                ...
                if not ref:
                    ...
                    try:
                        ...
                        if "VES" in moneda:
                            ...
                            if tasa_usada <= 1.0:
                                st.warning(
                                    f"⚠️ No se encontró tasa registrada para la fecha {fecha_p}. Se usará 1.0 por defecto."
                                )
                            else:
                        ...
                        with engine.connect() as conn:
                            ...
                            conn.commit()
                        st.success(
                            f"✅ Pago reportado con éxito. Equivalente calculado: ${monto_en_usd:,.2f} USD (Tasa: {tasa_usada:,.4f}). Queda en espera de verificación."
                        )
                        st.rerun()
                    except Exception as e:
